...
if ($id_sponsor && $name_sponsor) {
                                            if (($id_sponsor[0]->id_usuario != 1)) {
												...
												<div class="row">
													...
													<div class="col-xs-2 col-sm-1">
														<strong
															class="<?php if($actividad) echo "label label-success";else echo "label label-default";?>"
															style="font-size: 2rem;"> <?php if($actividad) echo "<i class='fa fa-smile-o'></i> Activo";else echo "<i class='fa fa-frown-o'></i> Inactivo";?></strong>
													</div>
										...
										<div class="col-sm-4">
											<h1>
												<small>Puntos Comisionables Personales</small> <i
													class='fa fa-user'></i>
											</h1>
											...
											<h1>
												<small>Puntos Comisionables Red </small> <i
											...
											<h1>
												<small>Últimos Auspiciados</small>
											</h1>
											<ul class="list-inline friends-list">
																<?php
                foreach ($ultimos_auspiciados as $afiliado) {
                    echo '<li><a onclick="detalles(' . $afiliado["id"] . ')">
                                <img src="' . $afiliado["foto"] . '"></a>
							</li>';
                }
                ?>
															</ul>
					...
					<div class="row">
						<div class="col-sm-12 col-md-12 col-lg-4">
							...
							<div class="well" style="">
								<!-- box-shadow: 0px 0px 0px !important;border-color: transparent; -->
								<fieldset>
									<legend>
										<b>Perfil y red</b>
									</legend>
...
$permiso = $ci->model_permissions->check($id, 'perfil');
            if ($permiso) {
                ?>
												<div class="col-sm-6">
											<a href="perfil_red/">
...
$permiso = $ci->model_permissions->check($id, 'reportes'); // afiliar
            if ($permiso) {
                ?>
												<div class="col-sm-6">
                ...
                ?>
						...
						<div class="col-sm-12 col-md-12 col-lg-4">
							...
							<div class="well">
								<fieldset>
									<legend>
										<b>Compras y comisiones</b>
									</legend>
									<div class="row">
												<?php
            
$permiso = $ci->model_permissions->check($id, 'carrito');
            if ($permiso) {
                ?>
												<div class="col-sm-6">
...
$permiso = $ci->model_permissions->check($id, 'billetera');
                        if ($permiso) {
                            ?>
												<div class="col-sm-6">
...
$permiso = $ci->model_permissions->check($id, 'afiliar'); // reportes
                        if ($permiso) {
                            ?>
												<div class="col-sm-6">
...
$permiso = $ci->model_permissions->check($id, 'tickets');
                    if ($permiso) {
                        ?>
												<div class="col-sm-6">
...
$permiso = $ci->model_permissions->check($id, 'encuestas');
            if ($permiso) {
                ?>
...
$permiso = $ci->model_permissions->check($id, 'tablero');
            if ($permiso) {
                ?>
												<div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">
									...
									<fieldset>
										...
										<div class="col-sm-2">
											<a href="javascript:void(0);">
												<div class="well well-sm txt-color-white text-center link_dashboard" style="background:<?=$style[0]->btn_2_color?>;">
													<div class="row">
														<i class="fa fa-facebook fa-1x"></i> <i
															class="fa fa-twitter fa-1x"></i>
													</div>
													<div class="row">
														<i class="fa fa-skype fa-1x"></i> <i
															class="fa fa-youtube fa-1x"></i>
													</div>
...
$permiso = $ci->model_permissions->check($id, 'mensajes');
            if ($permiso) {
                ?>
											<!--  	<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'chat');
            if ($permiso) {
                ?>
											<!--  	<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'e_mail');
            if ($permiso) {
                ?>
												<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'soporte_tecnico');
                if ($permiso) {
                    ?>
											<!--	<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'videollamadas');
                    if ($permiso) {
                        ?>
...
$permiso = $ci->model_permissions->check($id, 'social_network');
        if ($permiso) {
            ?>
											<!--		<div class="col-sm-12">
						...
						<div class="col-sm-12 col-md-12 col-lg-12">
							...
							<div class="well">
								<div class="row">
									<fieldset>
										<legend>
											<b>Información y capacitación</b>
										</legend>
										<div class="col-sm-2"></div>
												<?php
            
$permiso = $ci->model_permissions->check($id, 'informacion');
            if ($permiso) {
                ?>
												<div class="col-sm-2">
            ...
            $permiso = $ci->model_permissions->check($id, 'promociones');
            if ($permiso) {
                ?>
											<!--  	<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'eventos');
        if ($permiso) {
            ?>
											<div class="col-sm-2">
        ...
        $permiso = $ci->model_permissions->check($id, 'reconocimientos');
        if ($permiso) {
            ?>
										<!--  	<div class="col-sm-2">
...
$permiso = $ci->model_permissions->check($id, 'estadisticas');
                    if ($permiso) {
                        ?>
												<div class="col-sm-2">
