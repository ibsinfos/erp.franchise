...
class calculo
{
    ...
    {
        $this->insertarUsuario($this->id_usuario, $this->username, $this->created, $this->id_afiliacion, $this->id_red, $this->id_padre, $this->id_sponsor, $this->lado_red);
    }
    ...
    function getAfiliadosDebajoDe($id_afiliado, $red, $tipo, $nivel, $limite)
    {
        if ($limite > 0 || $nivel == 0) {
            ...
            foreach ($datos as $dato) {
                if ($dato != NULL) {
                    $idAfiliado = $dato["id_afiliado"];
                    array_push($this->idAfiliadosRed, $idAfiliado);
                    $this->setIdAfiliadosRed($this->idAfiliadosRed);
                    $this->getAfiliadosDebajoDe($idAfiliado, $red, $tipo, $nivel, $limite);
                }
            }
        }
    }
    ...
    function getAfiliadosDebajoDeBaseDeDatos($id_afiliado, $red, $tipo)
    {
        if ($tipo == "RED") {
            ...
            $data = "select A.id_afiliado as id_afiliado
								from afiliar A
								where A.debajo_de = " . $id_afiliado . " and A.id_red = " . $red;
